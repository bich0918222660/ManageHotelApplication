...
public class PaymentDetailDAO extends DAOAbstract<PaymentDetail> {
	...
	public List<PaymentDetail> getAll() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				list.add(new PaymentDetail(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getDouble(5),
						rs.getDouble(6)));
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean insert(PaymentDetail t) throws Exception {
		...
		if (t.getServiceID() == 0) {
			...
			sql = "insert into PaymentDetails(ServiceID, PaymentID, Quantity, Price, SubTotal) values (?,?,?,?,?)";
			ps = conn.prepareStatement(sql);
	...
	@Override
	public boolean update(PaymentDetail t) throws Exception {
		String sql = "";
		PreparedStatement ps;
		sql = "update PaymentDetails set ServiceID = ?, PaymentID = ?, "
				+ "Quantity = ?, Price = ?, SubTotal = ? where PaymentDetailID = ?";
		ps = conn.prepareStatement(sql);
		ps.setInt(1, t.getServiceID());
		ps.setInt(2, t.getPaymentID());
		ps.setInt(3, t.getQuantity());
		ps.setDouble(4, t.getPrice());
		ps.setDouble(5, t.getSubtotal());
		ps.setInt(6, t.getPaymentDetailID());
		return ps.executeUpdate() > 0;
	}
